...
async function createOrchestrationSystem() {
  const ServerConfigLayer = ServerConfig.layerTest(process.cwd(), {
    prefix: "ace-orchestration-engine-test-",
  });
    ...
    const layer = OrchestrationEngineLive.pipe(
      Layer.provide(
        Layer.succeed(ProjectionSnapshotQuery, {
          ...
          getThreadTimelinePages: () => Effect.succeed(Option.none()),
          getThreadTimelineManifest: () => Effect.succeed(Option.none()),
  ...
  it("stores completed checkpoint summaries even when no files changed", async () => {
    ...
    await system.run(
      engine.dispatch({
        ...
        checkpointRef: asCheckpointRef("refs/ace/checkpoints/thread-turn-diff/turn/1"),
        status: "ready",
