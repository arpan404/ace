...
type WorkspaceShikiTheme = (typeof WORKSPACE_SHIKI_THEMES)[keyof typeof WORKSPACE_SHIKI_THEMES];
type WorkspaceShikiLanguage = BundledLanguage | "text";
type WorkspaceShikiHighlighter = HighlighterGeneric<BundledLanguage, BundledTheme>;
...
const bundledLanguageLookup = bundledLanguages as Readonly<Record<string, unknown>>;
...
const defaultWorkspaceShikiHighlightConfig: WorkspaceShikiHighlightConfig = {
  ...
  language: null,
  theme: WORKSPACE_SHIKI_THEMES.light,
};
...
const workspaceShikiHighlightField = StateField.define<DecorationSet>({
  ...
  update(value, transaction) {
    ...
    let next = previousConfig.key === nextConfig.key ? value.map(transaction.changes) : Decoration.none;
...
export function createWorkspaceShikiHighlightConfig(input: {
  ...
  const theme = WORKSPACE_SHIKI_THEMES[input.resolvedTheme];
...
export function createWorkspaceShikiTokenStyle(input: TokenStyles): string | null {
  ...
  if (typeof input.fontStyle === "number" && input.fontStyle > 0) {
    ...
    if ((input.fontStyle & FONT_STYLE_STRIKETHROUGH) !== 0) {
      styleParts.push("text-decoration-line: line-through");
    }
  }
...
function mapWorkspaceLanguageToShikiLanguage(
  languageId: string,
  filePath: string,
): string | null {
  switch (languageId) {
...
function isBundledShikiLanguage(language: string): language is WorkspaceShikiLanguage {
  return language === "text" || Object.prototype.hasOwnProperty.call(bundledLanguageLookup, language);
}
...
async function getWorkspaceShikiHighlighter(
  ...
  highlighterPromise ??= createWorkspaceShikiHighlighter({
    langs: [],
    themes: [WORKSPACE_SHIKI_THEMES.dark, WORKSPACE_SHIKI_THEMES.light],
    warnings: false,
  });
