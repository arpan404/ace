...
export interface WsRpcClient {
  ...
  readonly shell: {
    ...
    readonly pathInfo: (input: {
      readonly path: Parameters<NativeApi["shell"]["pathInfo"]>[0];
    }) => ReturnType<NativeApi["shell"]["pathInfo"]>;
  };
  readonly filesystem: {
  ...
  readonly orchestration: {
    ...
    readonly stream: (
      input: RpcInput<typeof ORCHESTRATION_WS_METHODS.stream>,
      listener: Parameters<RpcStreamMethod<typeof ORCHESTRATION_WS_METHODS.stream>>[0],
    ) => () => void;
...
export function createWsRpcClient(transport: RpcTransportLike = new WsTransport()): WsRpcClient {
  ...
  return {
    ...
    shell: {
      ...
      pathInfo: (input) => transport.request((client) => client[WS_METHODS.shellPathInfo](input)),
    },
    filesystem: {
    ...
    orchestration: {
      ...
      replayEvents: (input) =>
        transport
          ...
          .then((events) => [...events]),
      stream: (input, listener) =>
        transport.subscribe((client) => client[ORCHESTRATION_WS_METHODS.stream](input), listener),
