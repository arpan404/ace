...
export const ConnectedChatComposerPanels = memo(function ConnectedChatComposerPanels(
  props: ConnectedChatComposerPanelsProps & {
    ref?: Ref<ConnectedChatComposerPanelsHandle>;
  },
) {
  const { ref } = props;
  ...
  const searchableModelOptions = useMemo(() => {
    ...
    for (const option of AVAILABLE_PROVIDER_OPTIONS) {
      if (props.lockedProvider !== null && option.value !== props.lockedProvider) {
        continue;
      }
      const providerOptions = props.modelOptionsByProvider[option.value] ?? [];
      for (const { slug, name } of providerOptions) {
        nextOptions.push({
  ...
  >(() => {
    ...
    if (composerTrigger.kind === "slash-command") {
      ...
      const providerCommandItems: Array<{
        id: string;
    ...
    >((items, option) => {
      ...
      if (
        ...
        items.push({
          id: `model:${option.provider}:${option.slug}`,
          type: "model",
          provider: option.provider,
          model: option.slug,
          label: option.name,
          description: `${option.providerLabel} · ${option.slug}`,
        });
      }
      return items;
    }, []);
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      const getPersistedAttachmentsForThread = () =>
        useComposerDraftStore.getState().draftsByThreadId[props.threadId]?.persistedAttachments ??
        [];
      try {
  ...
  const floatingDock = props.showFloatingDock ? (
    ...
    >
      <ChatComposerPanel
        ...
        selectedModelForPickerWithCustomFallback={props.selectedModelForPickerWithCustomFallback}
