...
export const PlanSummaryPanel = memo(function PlanSummaryPanel({
  ...
  return (
    <div className="flex min-h-0 flex-1 overflow-auto p-4">
      <section className="flex w-full min-w-0 flex-col overflow-hidden rounded-xl border border-border/60 bg-card/70">
              ...
              {(() => {
                ...
                return activePlan.steps.map((step, index) => {
                  ...
                  const isCurrentActionableStep =
                    planProgress?.currentIndex != null && index + 1 === planProgress.currentIndex;
                  return (
                    <div
                      key={stepKey}
                      className={cn(
                        "flex items-start gap-3 rounded-lg border border-border/55 bg-background/75 px-3 py-2.5 transition-colors duration-200",
