...
function CardFrame({ className, render, ...props }: useRender.ComponentProps<"div">) {
  const defaultProps = {
    className: cn(
      "[--clip-top:-1rem] [--clip-bottom:-1rem] *:data-[slot=card]:first:[--clip-top:1px] *:data-[slot=card]:last:[--clip-bottom:1px] relative flex flex-col rounded-lg border border-border bg-card text-card-foreground *:data-[slot=card]:-m-px *:not-last:data-[slot=card]:rounded-b-lg *:not-first:data-[slot=card]:rounded-t-lg *:data-[slot=card]:[clip-path:inset(var(--clip-top)_1px_var(--clip-bottom)_1px_round_calc(var(--radius-lg)-1px))]",
      className,
    ),
