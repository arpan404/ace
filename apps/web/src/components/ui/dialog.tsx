      ...
      >
        <DialogPrimitive.Popup
          className={cn(
            ...
            MODAL_SURFACE_CLASS_NAME,
            bottomStickOnMobile &&
              "max-sm:max-w-none max-sm:rounded-t-[1.15rem] max-sm:rounded-b-none max-sm:border-x-0 max-sm:border-t max-sm:border-b-0 max-sm:opacity-[calc(1-min(var(--nested-dialogs),1))] max-sm:data-ending-style:translate-y-4 max-sm:data-starting-style:translate-y-4",
            className,
          )}
            ...
            <DialogPrimitive.Close
              ...
              render={
                <Button
                  size="icon-sm"
                  variant="ghost"
                  className="rounded-lg text-muted-foreground/60 hover:bg-foreground/[0.055] hover:text-foreground/80"
                />
              }
            >
              <XIcon />
...
function DialogHeader({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      className={cn(
        MODAL_HEADER_CLASS_NAME,
        "pe-10 in-[[data-slot=dialog-popup]:has([data-slot=dialog-panel])]:pb-2 max-sm:pb-2",
        className,
      )}
  ...
  return (
    <ScrollArea scrollFade={scrollFade}>
      <div
        className={cn(
          MODAL_BODY_CLASS_NAME,
          "pb-4 in-[[data-slot=dialog-popup]:has([data-slot=dialog-header])]:pt-0 in-[[data-slot=dialog-popup]:has([data-slot=dialog-footer]:not(.border-t))]:pb-3",
          className,
        )}
