...
- `/apps/desktop`: Electron shell. Spawns a desktop-scoped `ace` backend process and loads the shared web app.
...
- `/packages/shared`: Shared runtime utilities consumed by both server and web. Uses explicit subpath exports (e.g. `@ace/shared/git`, `@ace/shared/DrainableWorker`) — no barrel index.
