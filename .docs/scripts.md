...
- Desktop production windows load the bundled UI from `ace://app/index.html` (not a `127.0.0.1` document URL).
- Desktop packaging includes `apps/server/dist` (the `ace` backend) and starts it on loopback with an auth token for WebSocket/API traffic.
...
Set `ACE_DEV_INSTANCE` to any value to deterministically shift all dev ports together.

- Default ports: server `3773`, web `5733`
- Shifted ports: `base + offset` (offset is hashed from `ACE_DEV_INSTANCE`)
- Example: `ACE_DEV_INSTANCE=branch-a bun run dev:desktop`

If you want full control instead of hashing, set `ACE_PORT_OFFSET` to a numeric offset.
