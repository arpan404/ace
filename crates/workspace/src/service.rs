use crate::{
    ...
    models::{
        ...
        ProjectWriteFileResult, ReadCacheEntry, ResolvedPath, WorkspaceEdit, WorkspaceEditResult,
        WorkspaceIndex,
    },
...
impl WorkspaceService {
    ...
    pub fn normalize_workspace_root(
        ...
        if !path.is_dir() {
            return Err(WorkspaceError::WorkspaceNotDirectory(path));
        }
        Ok(path)
    }
    ...
    pub fn create_entry(
        ...
        if target.absolute_path.exists() {
            return Err(WorkspaceError::EntryAlreadyExists(target.relative_path));
        }
    ...
    pub fn delete_entry(
        ...
        let target = Self::resolve_relative_path_within_root(&cwd, relative_path)?;
        if !target.absolute_path.exists() {
            return Err(WorkspaceError::EntryMissing(target.relative_path));
        }
    ...
    pub fn read_file(
        ...
        let contents =
            String::from_utf8(bytes.clone()).map_err(|_| WorkspaceError::UnsupportedFile)?;
    ...
    pub fn rename_entry(
        ...
        if !source.absolute_path.exists() {
            return Err(WorkspaceError::EntryMissing(source.relative_path));
        }
        if target.absolute_path.exists() {
            return Err(WorkspaceError::EntryAlreadyExists(target.relative_path));
        }
    ...
    pub fn apply_edit(
        ...
        let root = Self::normalize_workspace_root(&cwd, false)?;
        let mut written_files = Vec::with_capacity(edit.edits.len());
        for text_edit in &edit.edits {
            let target = Self::resolve_relative_path_within_root(&root, &text_edit.relative_path)?;
            let current = if target.absolute_path.exists() {
                self.read_file(&root, &target.relative_path)?.contents
            } else {
                String::new()
            };
            let next = if let Some(range) = &text_edit.range {
                replace_utf16_range(&current, range, &text_edit.new_text)?
            } else {
                text_edit.new_text.clone()
            };
            written_files.push(self.write_file(&root, &target.relative_path, &next, None, true)?);
        }
        Ok(WorkspaceEditResult { written_files })
    }
...
fn replace_utf16_range(
    current: &str,
    range: &crate::WorkspaceTextRange,
    new_text: &str,
) -> Result<String> {
    let buffer = crate::OpenBuffer::new("edit", current, 0);
    let start = buffer.utf16_position_to_char(range.start_line, range.start_character)?;
    let end = buffer.utf16_position_to_char(range.end_line, range.end_character)?;
    if start > end {
        return Err(WorkspaceError::InvalidEdit(
...
mod tests {
    ...
    fn writes_reads_and_conflicts_by_version() {
        ...
        let conflict = workspace
            ...
            .expect_err("conflict");
        assert!(matches!(conflict, WorkspaceError::VersionConflict { .. }));
    }
