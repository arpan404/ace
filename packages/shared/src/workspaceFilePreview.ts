import { parseRelayConnectionUrl } from "./relay";
...
function platformFromGlobal(): string {
  const navigatorValue = (globalThis as { navigator?: { platform?: unknown } }).navigator;
  return typeof navigatorValue?.platform === "string" ? navigatorValue.platform : "";
}

function normalizePlatform(platform?: string): string {
  return (platform ?? platformFromGlobal()).toLowerCase();
}
...
export function buildWorkspacePreviewHttpUrl(
  connectionUrl: string,
  cwd: string,
  relativePath: string,
): string | null {
  if (parseRelayConnectionUrl(connectionUrl)) {
    return null;
  }
  try {
    ...
    parsed.pathname = "/api/workspace-file";
    parsed.search = "";
    parsed.hash = "";
    parsed.searchParams.set("cwd", cwd);
    parsed.searchParams.set("relativePath", relativePath);
